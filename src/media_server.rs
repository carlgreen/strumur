...
fn generate_browse_albums_response(
    ...
    // TODO probably should be some 'all albums' pseudo-container, but this might do for now
    let update_id = collection.get_system_update_id();
...
fn generate_browse_an_album_response(
    ...
    let update_id = album.get_container_update_id();
...
fn generate_browse_items_response(
    ...
    // TODO probably should be some 'all items' pseudo-container, but this might do for now
    let update_id = collection.get_system_update_id();
...
fn generate_browse_artists_response(
    ...
    // TODO probably should be some 'artists' pseudo-container, but this might do for now
    let update_id = collection.get_system_update_id();
...
fn generate_browse_an_artist_response(
    ...
    let update_id = artist.get_container_update_id();
...
fn generate_browse_all_artists_response(
    ...
    // TODO probably should be some 'all artists' pseudo-container, but this might do for now
    let update_id = collection.get_system_update_id();
...
const REQUIRED_OBJECT_CONTAINER_PERSON_PROPERTIES: [&str; 0] = [];
const OPTIONAL_OBJECT_CONTAINER_PERSON_PROPERTIES: [&str; 1] = ["dc:language"];
...
fn generate_browse_artists_type_response(
    ...
    match options.browse_flag {
        BrowseFlag::Metadata => generate_browse_artists_metadata_response(collection),
...
fn generate_browse_all_artists_type_response(
    ...
    match options.browse_flag {
        BrowseFlag::Metadata => generate_browse_all_artists_metadata_response(collection),
...
fn generate_search_root_response(
    ...
    // we're searching from the root, so system update id is used
    let update_id = collection.get_system_update_id();
...
mod tests {
    ...
    fn make_track(
        ...
        Track::new(
            ...
            format!("{artist_name} feat. X"),
            format!("Music/{artist_name}/{album_title}/{track_number:02} {track_title}.flac")
                .replace(' ', "*20"),
            NaiveTime::from_hms_milli_opt(0, 2, 18, 893).unwrap(),
            18_323_574,
            16,
            44100,
            2,
    ...
    fn generate_test_collection() -> Collection {
        ...
        let j1 = make_album(0, &mut last_id, "jk", "j1", "1980-01-01", vec![]);
        let l1 = make_album(0, &mut last_id, "lm", "l1", "1982-02-02", vec![]);
        let n1 = make_album(0, &mut last_id, "nop", "n1", "1984-04-01", vec![]);
        let q1 = make_album(0, &mut last_id, "qrs", "q1", "1986-06-06", vec![]);
        let t1 = make_album(0, &mut last_id, "tuv", "t1", "1988-08-08", vec![]);
        let w1 = make_album(0, &mut last_id, "w", "w1", "1990-10-10", vec![]);
        let x1 = make_album(0, &mut last_id, "xyz", "x1", "1992-12-12", vec![]);

        Collection::new(
            7, // fun number for testing
            PathBuf::from("./"),
            vec![
                Artist::new(6, last_id + 1, "a<bc".to_string(), vec![a1]),
                Artist::new(
                    1,
                    last_id + 2,
                    "def".to_string(),
                ...
                Artist::new(15, last_id + 3, "ghi".to_string(), vec![g1, h2, i3]),
                Artist::new(1, last_id + 5, "jk".to_string(), vec![j1]),
                Artist::new(1, last_id + 5, "lm".to_string(), vec![l1]),
                Artist::new(1, last_id + 6, "nop".to_string(), vec![n1]),
                Artist::new(1, last_id + 7, "qrs".to_string(), vec![q1]),
                Artist::new(1, last_id + 8, "tuv".to_string(), vec![t1]),
                Artist::new(1, last_id + 9, "w".to_string(), vec![w1]),
                Artist::new(1, last_id + 10, "xyz".to_string(), vec![x1]),
            ],
        )
    }
...
<s:Envelope s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/" xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
    ...
    fn setup_test_write_test_data(filter: Filter) -> (BrowseOptions, Artist, Album, Track) {
        ...
        let track = Track::new(
            ...
            1,
            "some song".into(),
            "an artist feat. someone".into(),
            "01_some_song.flac".into(),
            NaiveTime::from_hms_opt(0, 3, 30).expect("should be a time"),
            1234,
            3,
            4,
            2,
        ...
        let artist = Artist::new(0, 1, "an artist".into(), vec![album.clone()]);
...
"#,
    ...
    fn test_generate_search_response_sort_by_class_descending_title_descending() {
        ...
        assert_eq!(
            result,
            r#"<DIDL-Lite xmlns="urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/" xmlns:dlna="urn:schemas-dlna-org:metadata-1-0/">
<item id="0$albums$*a9$*i6" parentID="0$albums$*a9" restricted="1"><dc:title>g&lt;11</dc:title><dc:date>1996-02-12</dc:date><upnp:album>g1</upnp:album><upnp:artist>ghi feat. X</upnp:artist><dc:creator>ghi feat. X</dc:creator><upnp:artist role="AlbumArtist">ghi</upnp:artist><upnp:originalTrackNumber>1</upnp:originalTrackNumber><res duration="0:02:18.893" size="18323574" bitsPerSample="16" sampleFrequency="44100" nrAudioChannels="2" protocolInfo="http-get:*:audio/x-flac:DLNA.ORG_OP=01;DLNA.ORG_FLAGS=01700000000000000000000000000000">abc/Music/ghi/g1/01*20g&lt;11.flac</res><upnp:class>object.item.audioItem.musicTrack</upnp:class></item>
<item id="0$albums$*a9$*i8" parentID="0$albums$*a9" restricted="1"><dc:title>g13</dc:title><dc:date>1996-02-12</dc:date><upnp:album>g1</upnp:album><upnp:artist>ghi feat. X</upnp:artist><dc:creator>ghi feat. X</dc:creator><upnp:artist role="AlbumArtist">ghi</upnp:artist><upnp:originalTrackNumber>3</upnp:originalTrackNumber><res duration="0:02:18.893" size="18323574" bitsPerSample="16" sampleFrequency="44100" nrAudioChannels="2" protocolInfo="http-get:*:audio/x-flac:DLNA.ORG_OP=01;DLNA.ORG_FLAGS=01700000000000000000000000000000">abc/Music/ghi/g1/03*20g13.flac</res><upnp:class>object.item.audioItem.musicTrack</upnp:class></item>
<item id="0$albums$*a9$*i7" parentID="0$albums$*a9" restricted="1"><dc:title>g12</dc:title><dc:date>1996-02-12</dc:date><upnp:album>g1</upnp:album><upnp:artist>ghi feat. X</upnp:artist><dc:creator>ghi feat. X</dc:creator><upnp:artist role="AlbumArtist">ghi</upnp:artist><upnp:originalTrackNumber>2</upnp:originalTrackNumber><res duration="0:02:18.893" size="18323574" bitsPerSample="16" sampleFrequency="44100" nrAudioChannels="2" protocolInfo="http-get:*:audio/x-flac:DLNA.ORG_OP=01;DLNA.ORG_FLAGS=01700000000000000000000000000000">abc/Music/ghi/g1/02*20g12.flac</res><upnp:class>object.item.audioItem.musicTrack</upnp:class></item>
<container id="0$albums$*a9" parentID="0$albums" childCount="3" restricted="1" searchable="1"><dc:title>g1</dc:title><dc:date>1996-02-12</dc:date><upnp:artist>ghi</upnp:artist><dc:creator>ghi</dc:creator><upnp:artist role="AlbumArtist">ghi</upnp:artist><upnp:albumArtURI dlna:profileID="JPEG_MED">abc/Music/ghi/g1/cover.jpg</upnp:albumArtURI><upnp:class>object.container.album.musicAlbum</upnp:class></container>
